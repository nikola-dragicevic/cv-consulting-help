...
import { Mail, Phone, FileText, Users, Award, Wand2, MapPin, Upload, X } from "lucide-react"
...
export default function CVConsultationService() {
  ...
  // Add this handler to avoid "Cannot find name 'handleFileUpload'" and to safely load .txt uploads
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    setError(null)
    const file = e.target.files?.[0]
    if (!file) return

    // Accept .txt files (some browsers set file.type empty for .txt)
    const isText = file.type === "text/plain" || file.name.toLowerCase().endsWith(".txt")
    if (!isText) {
      setError("Endast .txt-filer stöds.")
      // clear input so user can re-select same file later
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              ...
              <div className="sm:col-span-2">
                ...
                <div className="mt-2 flex items-center gap-2 text-xs text-slate-500">
                  <Upload className="h-3.5 w-3.5" /> Du kan även ladda upp .txt:
                  <Input type="file" accept=".txt" onChange={handleFileUpload} className="max-w-[240px]" />
                </div>
              </div>
            </div>
